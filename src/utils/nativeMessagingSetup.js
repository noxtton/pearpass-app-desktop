  ...
  try {
    ...
    switch (os.platform()) {
      case 'darwin':
        ...
        manifestPath = path.join(home, 'Library', 'Application Support', 'Google', 'Chrome', 'NativeMessagingHosts', manifestFile)
        break
        
      case 'win32':
        // Windows
        manifestPath = path.join(home, 'AppData', 'Local', 'PearPass', 'NativeMessaging', manifestFile)
        break
        
      case 'linux':
        // Linux
        manifestPath = path.join(home, '.config', 'google-chrome', 'NativeMessagingHosts', manifestFile)
        break
