...
describe('useGetMultipleFiles', () => {
    ...
    it('fetches files for each field and calls updateValues', async () => {
        ...
        await renderHook(() => useGetMultipleFiles({
    ...
    it('logs error if getFile throws', async () => {
        ...
        await waitFor(() => {
            expect(logger.error).toHaveBeenCalledWith('useGetMultipleFiles', 'Error retrieving files:', expect.any(Error));
            expect(mockUpdateValues).not.toHaveBeenCalled();
        });
    });
...
//# sourceMappingURL=useGetMultipleFiles.test.js.map