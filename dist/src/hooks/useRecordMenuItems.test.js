...
describe('useRecordMenuItems', () => {
    ...
    it('should include icons and colors in default items', () => {
        const { result } = renderHook(() => useRecordMenuItems());
        const loginItem = result.current.defaultItems.find((item) => item.type === RECORD_TYPES.LOGIN);
    ...
    it('should include "Password" in popupItems but not in defaultItems', () => {
        const { result } = renderHook(() => useRecordMenuItems());
        const passwordInPopup = result.current.popupItems.find((item) => item.type === 'password');
        const passwordInDefault = result.current.defaultItems.find((item) => item.type === 'password');
...
//# sourceMappingURL=useRecordMenuItems.test.js.map