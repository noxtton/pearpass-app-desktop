...
describe('useAnimatedVisibility', () => {
    test('initial state when isOpen is false', () => {
        const { result } = renderHook(() => useAnimatedVisibility({ isOpen: false }));
    ...
    test('transitions from closed to open', () => {
        const { result, rerender } = renderHook((props) => useAnimatedVisibility(props), { initialProps: { isOpen: false } });
        expect(result.current.isShown).toBe(false);
        expect(result.current.isRendered).toBe(false);
    ...
    test('transitions from open to closed', () => {
        const { result, rerender } = renderHook((props) => useAnimatedVisibility(props), { initialProps: { isOpen: true, transitionDuration: 300 } });
        expect(result.current.isShown).toBe(true);
        expect(result.current.isRendered).toBe(true);
    ...
    test('uses custom transition duration', () => {
        ...
        const { result, rerender } = renderHook((props) => useAnimatedVisibility(props), { initialProps: { isOpen: true, transitionDuration: customDuration } });
...
//# sourceMappingURL=useAnimatedVisibility.test.js.map