 ...
 */
export const useGetMultipleFiles = ({ fieldNames, updateValues, initialRecord } = {}) => {
    const getFilesAsync = async (fieldName) => {
        ...
        try {
            const files = await Promise.all(attachments.map(async (attachment) => {
                const file = await vaultGetFile(`record/${initialRecord.id}/file/${attachment.id}`);
...
//# sourceMappingURL=useGetMultipleFiles.js.map