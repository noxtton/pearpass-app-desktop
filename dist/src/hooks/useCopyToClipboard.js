...
const CLEAR_CLIPBOARD_PATH_PROD = Pear.config.applink + '/src/services/clipboardCleanup.js';
/**
 * @param {{
 *  onCopy?: () => void
 * }} props
...
export const useCopyToClipboard = ({ onCopy } = {}) => {
    const [isCopyToClipboardDisabled, setIsCopyToClipboardDisabled] = useState(false);
    ...
    const copyToClipboard = (text) => {
        ...
        navigator.clipboard.writeText(text).then(() => {
            ...
            pipeRef.current = pearRun(Pear.config.key ? CLEAR_CLIPBOARD_PATH_PROD : CLEAR_CLIPBOARD_PATH_DEV);
