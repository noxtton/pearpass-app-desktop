...
describe('Icon Components', () => {
    icons.forEach(({ name, Component }, index) => {
        test(`${name} renders correctly`, () => {
            const { container } = render(_jsx(Component, { size: "24px", color: "#000" }, index));
            ...
            const appliedColors = Array.from(elements).map((el) => el.getAttribute('stroke') || el.getAttribute('fill'));
