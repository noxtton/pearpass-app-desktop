...
export const generateNativeHostExecutable = async (executablePath) => {
    try {
        ...
        let content;
        if (platform === 'darwin') {
            const pearPath = path.join(os.homedir(), 'Library', 'Application Support', 'pear', 'current', 'by-arch', `${platform}-${arch}`, 'bin', 'pear-runtime');
            content = `#!/bin/bash
...
`;
            ...
            const pearPath = path.join(os.homedir(), '.config', 'pear', 'current', 'by-arch', `${platform}-${arch}`, 'bin', 'pear-runtime');
            content = `#!/bin/bash
...
`;
            ...
            const pearPath = path.join(os.homedir(), 'AppData', 'Roaming', 'pear', 'current', 'by-arch', `${platform}-${arch}`, 'bin', 'pear-runtime.exe');
            content = `@echo off
...
`;
        ...
        logger.info('NATIVE-MESSAGING-SETUP', `Generated native messaging executable at: ${executablePath}`);
        return {
            success: true,
        ...
        logger.error('NATIVE-MESSAGING-SETUP', `Failed to generate executable: ${error.message}`);
        return {
            success: false,
...
export const getNativeMessagingLocations = () => {
    ...
    switch (platform) {
        ...
        case 'win32':
            manifestPaths = [
                path.join(home, 'AppData', 'Local', 'PearPass', 'NativeMessaging', manifestFile)
            ];
            registryKeys = [
...
export const cleanupNativeMessaging = async () => {
    try {
        const { platform, manifestPaths, registryKeys } = getNativeMessagingLocations();
        let removedCount = 0;
        ...
        for (const manifestPath of manifestPaths) {
            try {
                ...
                removedCount++;
                logger.info('NATIVE-MESSAGING-CLEANUP', `Removed manifest file: ${manifestPath}`);
            }
            catch (err) {
                // File might not exist, which is fine
                if (err.code !== 'ENOENT') {
                    logger.error('NATIVE-MESSAGING-CLEANUP', `Failed to remove manifest at ${manifestPath}: ${err.message}`);
                }
            }
        }
        ...
        if (platform === 'win32') {
            for (const key of registryKeys) {
                ...
                try {
                    await execAsync(cmd);
                    logger.info('NATIVE-MESSAGING-CLEANUP', `Removed registry key: ${key}`);
                }
                catch (err) {
                    // Registry key might not exist, which is fine
                    logger.error('NATIVE-MESSAGING-CLEANUP', `Failed to remove registry key '${key}': ${err.message}`);
                }
            }
        }
    ...
    try {
        ...
        if (platform === 'win32') {
            ...
            try {
                ...
                logger.info('NATIVE-MESSAGING-KILL', 'Windows: Killed native messaging host processes');
            }
            catch (error) {
            ...
            try {
                ...
                logger.info('NATIVE-MESSAGING-KILL', 'Killed native messaging host process by bridge seed');
            }
            catch (error) {
                logger.info('NATIVE-MESSAGING-KILL', `No native messaging host process found to kill: ${error.message}`);
            }
        }
    }
    catch (error) {
        logger.error('NATIVE-MESSAGING-KILL', `Failed to kill host processes: ${error.message}`);
    }
};
/**
    ...
    try {
        ...
        for (const manifestPath of manifestPaths) {
            try {
                ...
                logger.error('NATIVE-MESSAGING-SETUP', `Failed to write manifest at ${manifestPath}: ${err.message}`);
            }
        }
        ...
        if (platform === 'win32') {
            ...
            for (const key of registryKeys) {
                ...
                try {
                    ...
                    logger.error('NATIVE-MESSAGING-SETUP', `Failed to write registry key '${key}': ${err.message}`);
                }
            }
        }
        return {
...
//# sourceMappingURL=nativeMessagingSetup.js.map