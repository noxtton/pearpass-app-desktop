...
describe('ToastContext', () => {
    ...
    it('should provide toast context', () => {
        ...
        render(_jsx(ToastProvider, { children: _jsx(TestComponent, { onToast: mockSetToast }) }));
    ...
    it('should add toast to stack when setToast is called', () => {
        ...
        render(_jsx(ToastProvider, { children: _jsx(TestComponent, { onToast: (fn) => {
                    setToastFn = fn;
                } }) }));
        act(() => {
        ...
        const callsWithToasts = Toasts.mock.calls.filter((call) => call[0].toasts.length > 0);
        expect(callsWithToasts.length).toBeGreaterThan(0);
        expect(callsWithToasts[callsWithToasts.length - 1][0]).toEqual(expect.objectContaining({
            toasts: [toastData]
        }));
    });
    ...
    it('should handle multiple toasts', () => {
        ...
        const callsWithTwoToasts = Toasts.mock.calls.filter((call) => call[0].toasts.length === 2);
        expect(callsWithTwoToasts.length).toBeGreaterThan(0);
        expect(callsWithTwoToasts[callsWithTwoToasts.length - 1][0]).toEqual(expect.objectContaining({
            toasts: [toast1, toast2]
        }));
    });
});
//# sourceMappingURL=ToastContext.test.js.map