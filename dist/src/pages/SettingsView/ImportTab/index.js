...
import { parse1PasswordData, parseBitwardenData, parseLastPassData, parseNordPassData, parsePearPassData, parseProtonPassData } from 'pearpass-lib-data-import';
...
const isAllowedType = (fileType, accepts) => accepts.some((accept) => {
        ...
        try {
            switch (type) {
                ...
                case 'unencrypted':
                    ...
                    break;
                default:
                    throw new Error('Unsupported template type. Please select a valid import option.');
            }
            if (result.length === 0) {
                setToast({
            ...
            logger.error('useGetMultipleFiles', 'Error reading file:', error.message || error);
        }
    };
    return html ` <${CardSingleSetting} title=${t('Import')}>
    <${ContentContainer}>
      <${Description}>
        ${t('Here you can import different file, export your data and then upload it here')}
      <//>

      <${ImportOptionsContainer}>
        ${importOptions.map(({ title, accepts, type, imgSrc, icon }) => html `<${ImportDataOption}
              key=${title}
              title=${title}
              accepts=${accepts}
              imgSrc=${imgSrc}
