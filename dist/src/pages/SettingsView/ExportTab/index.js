...
import { authoriseCurrentProtectedVault, getVaultById, getCurrentProtectedVaultEncryption, listRecords, useVault, useVaults, getMasterEncryption } from 'pearpass-lib-vault';
...
export const ExportTab = () => {
    ...
    const { isVaultProtected, refetch: refetchVault, data: currentVault } = useVault();
    ...
    const fetchProtectedVault = async (password, currentVaultId, currentEncryption) => {
        if (selectedProtectedVault?.id &&
    ...
    const handleVaultClick = async (vault) => {
        ...
        if (isProtected) {
            setSelectedProtectedVault(selectedProtectedVault?.id === vault.id ? null : vault);
            setSelectedVaults([]);
            return;
        }
        setSelectedVaults((prev) => prev.includes(vault.id)
    ...
    const handleExport = async () => {
        ...
        if (selectedProtectedVault) {
            setModal(html `<${VaultPasswordFormModalContent}
          vault=${selectedProtectedVault}
          onSubmit=${async (password) => {
                await fetchProtectedVault(password, currentVaultId, currentEncryption);
                closeModal();
            }}
        />`);
        }
        ...
        >
          <${AuthenticationCard}
            ...
            descriptionComponent=${html `<${AlertBox}
              message=${t('Confirm your master password to export your vault data.')}
            />`}
            style=${{ width: '100%' }}
            onSuccess=${async () => {
                const vaultsToExport = await Promise.all(selectedVaults.map(async (vaultId) => await fetchUnprotectedVault(vaultId)));
    ...
    <${ContentContainer}>
      <${Description}>
        ${t('Choose which Vaults do you want to backup and select if you want the file encrypted')}
      <//>
      ${data?.map((vault) => html `<${ListItem}
            key=${vault.name}
            itemName=${vault.name}
