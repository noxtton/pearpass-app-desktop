...
import { LearnMoreLink, ListContainer, TooltipContent, TooltipText, Wrapper } from './styles';
...
export const SettingsBlindPeersSection = () => {
    ...
    const { removeAllBlindMirrors, data: blindMirrorsData, getBlindMirrors, addBlindMirrors, addDefaultBlindMirrors } = useBlindMirrors();
     ...
     * }} params
     */
    const handleBlindMirrorsRequest = async ({ callback, errorMessage, successMessage }) => {
        try {
            setIsLoading(true);
            await callback();
    ...
    const handleSetBlindPeersRules = async (newRules) => {
        if (newRules.blindPeers === true) {
            setModal(html `<${BlindPeersModalContent}
            ...
            <${TooltipWrapper}>
              <${TooltipContent}>
                <${TooltipText}>
                  ...
                  <${ListContainer}>
                    <li>
                      ${t('Automatic blind peers: Let PearPass allocate blind peers for you to handle syncing.')}
                    </li>
                    <li>
                      ${t('Manual blind peers: Setup your own private blind peers.')}
                    </li>
                  <//>
