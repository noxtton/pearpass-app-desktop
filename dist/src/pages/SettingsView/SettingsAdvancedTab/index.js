...
export const SettingsAdvancedTab = () => {
    ...
    const { isBrowserExtensionEnabled, toggleBrowserExtension } = useConnectExtension();
    const [isPasswordReminderDisabled, setIsPasswordReminderDisabled] = useState(isPasswordChangeReminderDisabled());
    const [selectedRules, setSelectedRules] = useState(() => {
        const isDisabled = localStorage.getItem(LOCAL_STORAGE_KEYS.COPY_TO_CLIPBOARD_DISABLED);
        return {
    ...
    const handlePasswordChangeReminder = (isEnabled) => {
        if (!isEnabled) {
            localStorage.setItem(LOCAL_STORAGE_KEYS.PASSWORD_CHANGE_REMINDER_ENABLED, 'false');
        }
        else {
