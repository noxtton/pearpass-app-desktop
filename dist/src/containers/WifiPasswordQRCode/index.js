 ...
 * }} props
 */
export const WifiPasswordQRCode = ({ ssid, password, encryptionType = 'WPA', isHidden = false }) => {
    const { i18n } = useLingui();
    const [qrCodeSvg, setQrCodeSvg] = useState('');
    const generateWifiQRString = (ssid, password, encryptionType = 'WPA', isHidden = false) => `WIFI:T:${encryptionType};S:${ssid};P:${password};H:${isHidden};;`;
    useEffect(() => {
        if (ssid && password) {
            const wifiString = generateWifiQRString(ssid, password, encryptionType, isHidden);
