...
import { AddContainer, Body, Container, DeleteIconWrapper, DeleteOverlay, Header, ImageContainer, Photo, Title } from './styles';
...
export const ImagesField = ({ title, pictures = [], onAdd, onRemove }) => {
    ...
    const pictureUrls = useMemo(() => pictures.map((picture) => ({
    ...
    <${Container}>
      ...
      <${Body}>
        ${pictureUrls?.map((picture, idx) => html `
            <${ImageContainer}
