...
import { closeAllInstances, useFolders, useVault, useVaults } from 'pearpass-lib-vault';
import { SideBarCategories } from './SidebarCategories';
import { SidebarNestedFolders } from './SidebarNestedFolders';
import { FoldersWrapper, LogoWrapper, PearPass, SettingsContainer, SettingsSeparator, sideBarContent, SidebarNestedFoldersContainer, SidebarSettings, SidebarWrapper } from './styles';
...
import { ButtonThin, ExitIcon, SettingsIcon, StarIcon, UserSecurityIcon } from '../../lib-react-components';
...
export const Sidebar = ({ sidebarSize = 'tight' }) => {
    ...
    const { data: vaultsData, resetState, refetch: refetchMasterVault } = useVaults();
    const { data: vaultData } = useVault();
    const vaults = useMemo(() => vaultsData?.filter((vault) => vault.id !== vaultData?.id), [vaultsData, vaultData]);
    ...
    const matchesSearch = (records, searchValue) => {
        ...
        return records.some((record) => matchPatternToValue(searchValue, record?.data?.title ?? ''));
    };
        ...
        return [
            {
                ...
                children: favorites?.records?.map((record) => ({
        ...
        html `
          <${SidebarNestedFoldersContainer}>
            ...
            <${FoldersWrapper} data-testid="sidebar-folders-container">
              ${folders.map((folder) => html `<${SidebarNestedFolders}
                    item=${folder}
