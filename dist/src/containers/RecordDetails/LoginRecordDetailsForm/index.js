...
import { CompoundField, CopyIcon, InputField, KeyIcon, PasswordField, UserIcon, WorldIcon } from '../../../lib-react-components';
...
export const LoginRecordDetailsForm = ({ initialRecord, selectedFolder }) => {
    ...
    const { value: customFieldsList, registerItem: registerCustomFieldItem } = registerArray('customFields');
    useGetMultipleFiles({
    ...
    <${FormWrapper}>
          ...
          ${websitesList.map((website, index) => html `
              <${React.Fragment} key=${website.id}>
                <${InputField}
                  ...
                  isDisabled
                  onClick=${() => handleWebsiteClick(registerItem('website', index).value)}
                />
              <//>
            `)}
        <//>
      ...
      ${values?.attachments?.length > 0 &&
        html `
        <${FormGroup}>
          ${values.attachments.map((attachment) => html `
              <${AttachmentField}
                label=${i18n._('File')}
