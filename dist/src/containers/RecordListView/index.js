...
import { ActionsSection, DatePeriod, Folder, LeftActions, RecordsSection, RightActions, ViewWrapper } from './styles';
...
import { ArrowUpAndDown, ButtonFilter, DeleteIcon, FolderIcon, MoveToIcon, MultiSelectionIcon, StarIcon, TimeIcon, XIcon } from '../../lib-react-components';
 ...
 * }} props
 */
export const RecordListView = ({ records, selectedRecords, setSelectedRecords, sortType, setSortType }) => {
    ...
    const selectedSortAction = sortActions.find((action) => action.type === sortType);
    ...
    const handleSelect = (record, isSelected) => {
        ...
        setSelectedRecords((prev) => isSelected
    ...
    <${ViewWrapper}>
      ...
      <${RecordsSection}>
        ...
        const isSelected = selectedRecords.some((selectedRecord) => selectedRecord.id === record?.id);
        const isStartOfLast7Days = isStartOfLast7DaysGroup(record, index, records);
        const isStartOfLast14Days = isStartOfLast14DaysGroup(record, index, records);
        return html `
