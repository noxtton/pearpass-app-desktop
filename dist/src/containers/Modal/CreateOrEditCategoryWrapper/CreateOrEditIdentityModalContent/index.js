...
import { ButtonLittle, ButtonSingleInput, CalendarIcon, DeleteIcon, EmailIcon, GenderIcon, GroupIcon, ImageIcon, InputField, NationalityIcon, PhoneIcon, SaveIcon, UserIcon } from '../../../../lib-react-components';
 ...
 * }} props
 */
export const CreateOrEditIdentityModalContent = ({ initialRecord, selectedFolder, onTypeChange }) => {
    ...
    const schema = Validator.object({
        ...
        customFields: Validator.array().items(Validator.object({
        ...
        passportPicture: Validator.array().items(Validator.object({
            id: Validator.string(),
        ...
        idCardPicture: Validator.array().items(Validator.object({
            id: Validator.string(),
            name: Validator.string().required()
        })),
        ...
        drivingLicensePicture: Validator.array().items(Validator.object({
            id: Validator.string(),
            name: Validator.string().required()
        })),
    ...
    const { value: list, addItem, registerItem, removeItem } = registerArray('customFields');
    useGetMultipleFiles({
        fieldNames: [
    ...
    const handleFileLoad = (fieldName) => {
        setModal(html `<${UploadFilesModalContent}
        type=${'file'}
        onFilesSelected=${(files) => handleFileSelect({
            files,
            fieldName,
            setValue,
            values
    ...
    const handleAttachmentRemove = (fieldName, index) => {
        const updatedAttachments = values[fieldName].filter((_, idx) => idx !== index);
      ...
      <${FormWrapper}>
        ...
        >
          ...
          <${ImagesField}
            ...
            onRemove=${(index) => handleAttachmentRemove('passportPicture', index)}
          />
        <//>
        ...
        >
          ...
          <${ImagesField}
            ...
            onRemove=${(index) => handleAttachmentRemove('idCardPicture', index)}
          />
        <//>
        ...
        >
          ...
          <${ImagesField}
            ...
            onRemove=${(index) => handleAttachmentRemove('drivingLicensePicture', index)}
          />
        <//>
        ...
        html `
          <${FormGroup}>
            ${values.attachments.map((attachment, index) => html `<${AttachmentField}
                  ...
                  additionalItems=${html `
                    <${ButtonSingleInput}
                      ...
                      onClick=${() => handleAttachmentRemove(ATTACHMENTS_FIELD_KEY, index)}
                    >
                      ${i18n._('Delete File')}
                    <//>
                  `}
