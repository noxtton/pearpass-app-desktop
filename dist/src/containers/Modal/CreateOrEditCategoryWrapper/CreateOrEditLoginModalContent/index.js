...
import { ButtonLittle, ButtonRoundIcon, ButtonSingleInput, CompoundField, DeleteIcon, ImageIcon, InputField, KeyIcon, PasswordField, PasswordIcon, PlusIcon, SaveIcon, UserIcon, WorldIcon } from '../../../../lib-react-components';
 ...
 * }} props
 */
export const CreateOrEditLoginModalContent = ({ initialRecord, selectedFolder, onTypeChange }) => {
    ...
    const schema = Validator.object({
        ...
        websites: Validator.array().items(Validator.object({
    ...
    const { value: websitesList, addItem, registerItem, removeItem } = registerArray('websites');
    const { value: customFieldsList, addItem: addCustomField, registerItem: registerCustomFieldItem, removeItem: removeCustomFieldItem } = registerArray('customFields');
    useGetMultipleFiles({
    ...
    >
      <${FormWrapper}>
        ...
        <${CompoundField}>
          ${websitesList.map((website, index) => html `
        ...
        html `
          <${FormGroup}>
            ${values.attachments.map((attachment) => html `<${AttachmentField}
                  ...
                  additionalItems=${html `
                    <${ButtonSingleInput}
                      ...
                      startIcon=${DeleteIcon}
                      onClick=${() => setValue(ATTACHMENTS_FIELD_KEY, getFilteredAttachmentsById(values.attachments, attachment))}
                    >
                      ${i18n._('Delete File')}
                    <//>
                  `}
        ...
        <${FormGroup}>
          <${CreateCustomField}
            ...
            onCreateCustom=${(type) => addCustomField({ type: type, name: type })}
          />
        <//>
      <//>
    <//>
