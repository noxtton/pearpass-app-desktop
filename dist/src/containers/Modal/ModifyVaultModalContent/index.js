...
import { ButtonPrimary, ButtonSecondary, PearPassInputField, PearPassPasswordField } from '../../../lib-react-components';
import { logger } from '../../../utils/logger';
import { ModalContent } from '../ModalContent';
import { Content, InputLabel, InputWrapper, ModalActions, ModalTitle, Wrapper } from './styles';
const UPDATE_MODE = {
    NAME: 'name',
    PASSWORD: 'password'
};
export const ModifyVaultModalContent = ({ vaultId, vaultName }) => {
    ...
    const { isVaultProtected, updateUnprotectedVault, updateProtectedVault, refetch: refetchVault } = useVault();
    const [isProtected, setIsProtected] = useState(false);
    const [selectedOption, setSelectedOption] = useState(UPDATE_MODE.NAME);
    const { setIsLoading } = useLoadingContext();
    const radioOptions = useMemo(() => PROTECTED_VAULT_ENABLED
        ? [
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const password = selectedOption === UPDATE_MODE.PASSWORD ? values.newPassword : undefined;
            if (isProtected) {
