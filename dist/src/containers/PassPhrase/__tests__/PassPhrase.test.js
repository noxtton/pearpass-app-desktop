...
// Styles
jest.mock('../styles', () => ({
    Container: ({ children }) => _jsx("div", { "data-testid": "container", children: children }),
    PassPhraseHeader: ({ children }) => (_jsx("div", { "data-testid": "header", children: children })),
    HeaderText: ({ children }) => _jsx("h1", { "data-testid": "title", children: children }),
    CopyPasteButton: ({ children, onClick, type }) => (_jsx("button", { "data-testid": "copy-paste", onClick: onClick, type: type, children: children })),
    CopyPasteText: ({ children }) => (_jsx("span", { "data-testid": "copy-paste-text", children: children })),
    PassPhraseContainer: ({ children }) => (_jsx("div", { "data-testid": "words", children: children })),
    ErrorContainer: ({ children }) => (_jsx("div", { "data-testid": "error-container", children: children })),
    ErrorText: ({ children }) => _jsx("div", { "data-testid": "error-text", children: children })
}));
// Icons and shared components
jest.mock('../../../lib-react-components', () => ({
    CopyIcon: () => _jsx("i", { "data-testid": "copy-icon" }),
    PasteIcon: () => _jsx("i", { "data-testid": "paste-icon" }),
    PassPhraseIcon: () => _jsx("i", { "data-testid": "pp-icon" }),
    ErrorIcon: () => _jsx("i", { "data-testid": "error-icon" })
}));
// Badge item capture
...
jest.mock('../../../components/BadgeTextItem', () => ({
    BadgeTextItem: (props) => {
        ...
        return (_jsxs("div", { "data-testid": "badge-item", children: ["#", props.count, ":", props.word] }));
    }
}));
...
jest.mock('../PassPhraseSettings', () => ({
    PassPhraseSettings: (props) => {
        ...
        return _jsx("div", { "data-testid": "settings" });
    }
}));
// Hooks
...
describe('PassPhrase (container)', () => {
    const renderComponent = (props = {}) => render(_jsx(ThemeProvider, { children: _jsx(PassPhrase, { isCreateOrEdit: false, onChange: jest.fn(), value: "", error: "", ...props }) }));
    beforeEach(() => {
    ...
    test('paste button pastes, updates list, and calls onChange in create/edit', async () => {
        ...
        const value = 'fo1o-1bar-3bam,.z-q#ux-quux-corge-gra.!ult-garpl&&!@y-waldo-!red-pl@ugh-%@xyzzy';
    ...
    test('13 words: switch is on and radio selected to 12', () => {
        const thirteen = 'one two three four five six seven eight nine ten eleven twelve rand';
    ...
    test('after paste: settings are disabled', async () => {
        mockPaste.mockResolvedValueOnce('one two three four five six seven eight nine ten eleven twelve');
        renderComponent({ isCreateOrEdit: true });
        fireEvent.click(screen.getByTestId('copy-paste'));
        await waitFor(() => expect(mockBadgeCalls.length).toBe(12));
        const last = mockSettingsCalls[mockSettingsCalls.length - 1];
