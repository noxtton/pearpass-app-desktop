...
import { PASSPHRASE_WORD_COUNTS, VALID_WORD_COUNTS, DEFAULT_SELECTED_TYPE } from 'pearpass-lib-constants';
import { colors } from 'pearpass-lib-ui-theme-provider';
import { PassPhraseSettings } from './PassPhraseSettings';
import { Container, PassPhraseHeader, HeaderText, CopyPasteButton, CopyPasteText, PassPhraseContainer, ErrorContainer, ErrorText } from './styles';
...
import { CopyIcon, PassPhraseIcon, ErrorIcon, PasteIcon } from '../../lib-react-components';
/**
 * @param {{
 ...
 * }} props
 */
export const PassPhrase = ({ isCreateOrEdit, onChange, value, error, testId }) => {
    ...
    const parsePassphraseText = (text) => text
        .trim()
        .split(/[-\s]+/)
        .map((word) => word.trim())
    ...
    const detectAndUpdateSettings = (words) => {
        ...
        if (wordCount === PASSPHRASE_WORD_COUNTS.STANDARD_12 ||
        ...
        else if (wordCount === PASSPHRASE_WORD_COUNTS.STANDARD_24 ||
    ...
    const isValidRange = (wordCount) => !wordCount || VALID_WORD_COUNTS.includes(wordCount);
    ...
    const isCreateOrEditWithValidRange = isCreateOrEdit && isValidRange(passphraseWords.length);
    return html `
    <${Container} data-testid=${testId}>
      ...
      <${PassPhraseContainer}>
        ${passphraseWords.map((word, i) => html `<${BadgeTextItem}
              key=${`${word}-${i}`}
              count=${i + 1}
              word=${word || ''}
            />`)}
