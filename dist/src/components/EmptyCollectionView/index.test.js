...
describe('EmptyCollectionView component', () => {
    const renderComponent = () => render(_jsx(ThemeProvider, { children: _jsx(EmptyCollectionView, {}) }));
    ...
    test('displays help text', () => {
        const { getByText } = renderComponent();
        expect(getByText('Create a new element or pass to another collection')).toBeInTheDocument();
    });
});
