...
import { CollectionsContainer, CollectionsTitle, CollectionsWrapper } from './styles';
...
export const EmptyCollectionView = () => {
    ...
    <${CollectionsWrapper}>
      <${CollectionsContainer}>
        ...
        .filter((option) => data.recordType === 'all' || option.type === data.recordType)
        .map((option) => html `
              <${ButtonCreate}
                key=${option.type}
                startIcon=${RECORD_ICON_BY_TYPE[option.type]}
                onClick=${() => handleCreateOrEditRecord({ recordType: option.type })}
              >
                ${option.text}
              <//>
            `)}
