...
jest.mock('../../constants/recordActions', () => ({
    RECORD_ACTION_ICON_BY_TYPE: {
        edit: () => _jsx("svg", { "data-testid": "edit-icon" }),
        delete: () => _jsx("svg", { "data-testid": "delete-icon" })
    }
}));
describe('RecordActionsPopupContent Component', () => {
    ...
    test('renders correctly with default variant', () => {
        const { asFragment, getByText } = render(_jsx(ThemeProvider, { children: _jsx(RecordActionsPopupContent, { menuItems: mockMenuItems }) }));
        expect(getByText('Edit')).toBeInTheDocument();
        expect(getByText('Delete')).toBeInTheDocument();
        expect(asFragment()).toMatchSnapshot();
    });
    test('renders correctly with compact variant', () => {
        const { asFragment } = render(_jsx(ThemeProvider, { children: _jsx(RecordActionsPopupContent, { menuItems: mockMenuItems, variant: "compact" }) }));
        expect(asFragment()).toMatchSnapshot();
    });
    test('calls item.click when menu item is clicked', () => {
        const { getByText } = render(_jsx(ThemeProvider, { children: _jsx(RecordActionsPopupContent, { menuItems: mockMenuItems }) }));
    ...
    test('calls onClick prop when menu item without click handler is clicked', () => {
        ...
        const { getByText } = render(_jsx(ThemeProvider, { children: _jsx(RecordActionsPopupContent, { menuItems: itemsWithoutClick, onClick: onClickMock }) }));
        fireEvent.click(getByText('Edit'));
