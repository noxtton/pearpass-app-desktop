...
import { ArrowIconWrapper, Container, Dropdown, DropdownItem, Wrapper } from './styles';
import { VaultPasswordFormModalContent } from '../../containers/Modal/VaultPasswordFormModalContent';
import { useModal } from '../../context/ModalContext';
import { ArrowDownIcon, ArrowUpIcon, LockCircleIcon } from '../../lib-react-components';
...
export const DropdownSwapVault = ({ vaults, selectedVault }) => {
    ...
    const onVaultSelect = async (vault) => {
        ...
        if (isProtected) {
          ...
          onSubmit=${async (password) => await handleVaultUnlock({ vault, password })}
          vault=${vault}
        />`);
        }
        else {
