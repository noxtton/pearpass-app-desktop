...
describe('CreateCustomField component', () => {
    ...
    test('renders with correct initial state', () => {
        const { container } = render(_jsx(ThemeProvider, { children: _jsx(CreateCustomField, { onCreateCustom: mockOnCreateCustom }) }));
        expect(container).toMatchSnapshot();
    });
    test('opens dropdown when clicked', () => {
        const { getByText } = render(_jsx(ThemeProvider, { children: _jsx(CreateCustomField, { onCreateCustom: mockOnCreateCustom }) }));
    ...
    test('closes dropdown after selecting an option', () => {
        const { getByText, queryByText } = render(_jsx(ThemeProvider, { children: _jsx(CreateCustomField, { onCreateCustom: mockOnCreateCustom }) }));
        fireEvent.click(getByText('Create Custom'));
        fireEvent.click(getByText('Note'));
