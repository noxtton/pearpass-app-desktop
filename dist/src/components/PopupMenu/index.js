...
import { MenuCard, MenuTrigger, MenuWrapper, TRANSITION_DURATION } from './styles';
 ...
 * }} props
 */
export const PopupMenu = ({ isOpen, setIsOpen, children, content, direction = 'bottomLeft', displayOnHover = false }) => {
    ...
    const { newDirection, newPositions } = useMemo(() => {
        const { right = 0, left = 0, top = 0, bottom = 0 } = boxRef.current?.getBoundingClientRect() || {};
        const width = boxRef.current?.children[0]?.getBoundingClientRect().width ?? 0;
        const height = boxRef.current?.children[0]?.getBoundingClientRect().height ?? 0;
        ...
        const newPositions = {
            ...
            bottom: bottomPosition - (positionToSet.vertical === 'bottom' ? height : 0),
            width: width,
            height: height
        };
        ...
        return {
            newDirection: `${positionToSet.vertical}${positionToSet.vertical
    ...
    const contentOrigin = useMemo(() => {
        ...
        const { top = 0, bottom = 0, left = 0, right = 0, width = 0, height = 0 } = wrapperRef.current.getBoundingClientRect() || {};
        switch (newDirection) {
            case 'top':
                return {
                    top: top,
