...
describe('MethodRegistry', () => {
    ...
    describe('execute', () => {
        it('should throw UnknownMethod for unregistered methods', async () => {
            await expect(registry.execute('unknownMethod', {}, context)).rejects.toThrow('UnknownMethod: unknownMethod');
        });
        it('should execute registered methods', async () => {
            ...
            const result = await registry.execute('testMethod', { test: 'param' }, context);
        ...
        describe('authentication checks', () => {
            ...
            it('should check authentication for non-exempt methods', async () => {
                ...
                await expect(registry.execute('secureMethod', {}, context)).rejects.toThrow('DesktopNotAuthenticated: Desktop app is not authenticated');
            ...
            it('should skip auth check for encryptionGet', async () => {
                ...
                // Don't mock vaultsGetStatus - it shouldn't be called
                const result = await registry.execute('encryptionGet', { key: 'masterPassword' }, context);
        ...
        describe('logging', () => {
            it('should log debug messages when configured', async () => {
                ...
                expect(logger.debug).toHaveBeenCalledWith('METHOD-REGISTRY', expect.stringContaining('debugMethod'));
            });
            it('should log errors', async () => {
                const handler = jest.fn().mockRejectedValue(new Error('Handler error'));
                registry.register('errorMethod', handler);
                mockClient.vaultsGetStatus.mockResolvedValue({ status: true });
                await expect(registry.execute('errorMethod', {}, context)).rejects.toThrow('Handler error');
                expect(logger.error).toHaveBeenCalledWith('METHOD-REGISTRY', 'Error in errorMethod: Handler error');
            });
        });
    });
