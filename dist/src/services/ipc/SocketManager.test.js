...
describe('SocketManager', () => {
    ...
    describe('cleanupSocket', () => {
        ...
        it('calls unlink and logs info on Unix', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('SOCKET-MANAGER', 'Cleaned up existing socket file');
        });
        it('logs warn if unlink throws non-ENOENT error', async () => {
            ...
            const manager = new SocketManager(socketName);
            await manager.cleanupSocket();
            expect(logger.warn).toHaveBeenCalledWith('SOCKET-MANAGER', expect.stringContaining('Could not clean up socket file'));
        });
...
//# sourceMappingURL=SocketManager.test.js.map