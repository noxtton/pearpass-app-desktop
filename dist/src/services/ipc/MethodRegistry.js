...
export class MethodRegistry {
    ...
    async execute(methodName, params, context) {
        ...
        if (!handler) {
            ...
            logger.error('METHOD-REGISTRY', `Available methods: ${availableMethods.join(', ')}`);
        ...
        if (shouldCheckAuth) {
            try {
                ...
                logger.info('METHOD-REGISTRY', `vaultsStatusRes ${JSON.stringify(vaultsStatusRes)}`);
                if (!!!vaultsStatusRes?.status) {
                    logger.info('METHOD-REGISTRY', `Desktop not authenticated for method ${methodName}`);
                ...
                logger.info('METHOD-REGISTRY', `Could not verify auth for ${methodName}: ${error.message}`);
                throw new Error('DesktopNotAuthenticated: Desktop app is not authenticated');
            }
        }
        ...
        try {
            ...
            logger.error('METHOD-REGISTRY', `Error in ${methodName}: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async performStatusChecks(methodName, statusChecks, context) {
        ...
        if (Object.keys(statuses).length > 0) {
            logger.debug('METHOD-REGISTRY', `Status before ${methodName}: ${JSON.stringify(statuses)}`);
        }
    }
    /**
