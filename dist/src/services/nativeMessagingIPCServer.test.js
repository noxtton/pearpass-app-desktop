...
import { getIpcPath, getIPCSocketPath, isNativeMessagingIPCRunning, NativeMessagingIPCServer, startNativeMessagingIPC, stopNativeMessagingIPC } from './nativeMessagingIPCServer.js';
...
describe('nativeMessagingIPCServer', () => {
    ...
    describe('NativeMessagingIPCServer', () => {
        ...
        it('should construct with initial state', () => {
            ...
            expect(serverInstance.socketPath).toBe(join('/tmp', 'pearpass-native-messaging.sock'));
        });
        describe('start', () => {
            it('should start the IPC server successfully', async () => {
                ...
                expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'Starting native messaging IPC server...');
                expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', `Native messaging IPC server started successfully on ${serverInstance.socketPath}`);
            });
            it('should not start if already running', async () => {
                serverInstance.isRunning = true;
                await serverInstance.start();
                expect(IPC.Server).not.toHaveBeenCalled();
                expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'IPC server is already running');
            });
            it('should handle and log errors during startup', async () => {
                ...
                expect(logger.error).toHaveBeenCalledWith('IPC-SERVER', `Failed to start IPC server: ${error.message}`);
            });
        ...
        describe('stop', () => {
            it('should stop the server if it is running', async () => {
                ...
                expect(serverInstance.isRunning).toBe(false);
                expect(serverInstance.server).toBeNull();
                expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'Stopping native messaging IPC server...');
                expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'Native messaging IPC server stopped');
            });
            it('should not do anything if the server is not running', async () => {
                await serverInstance.stop();
                expect(logger.info).not.toHaveBeenCalledWith('IPC-SERVER', 'Stopping native messaging IPC server...');
            });
        });
    });
    describe('Singleton Functions', () => {
        ...
        it('startNativeMessagingIPC should return the existing instance if already running', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'Native messaging IPC server is already running');
        });
        ...
        it('stopNativeMessagingIPC should do nothing if not running', async () => {
            await stopNativeMessagingIPC();
            expect(logger.info).toHaveBeenCalledWith('IPC-SERVER', 'Native messaging IPC server is not running');
        });
        ...
        it('getIPCSocketPath should return a default path when not running', () => {
            platform.mockReturnValue('linux');
            expect(getIPCSocketPath()).toBe(join('/tmp', 'pearpass-native-messaging.sock'));
        });
    });
});
//# sourceMappingURL=nativeMessagingIPCServer.test.js.map