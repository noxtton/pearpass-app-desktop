...
export class VaultHandlers {
    ...
    async vaultsInit(params) {
        logger.debug('VAULT-HANDLER', `vaultsInit called with encryptionKey: ${params?.encryptionKey ? 'provided' : 'missing'}`);
        // Initialize vaults
    ...
    async vaultsList(params) {
        logger.debug('VAULT-HANDLER', `vaultsList called with filterKey: ${params?.filterKey}`);
        // Check vault status first
        const vaultsStatus = await this.client.vaultsGetStatus();
        logger.debug('VAULT-HANDLER', `Vaults status before list: ${JSON.stringify(vaultsStatus)}`);
        const result = await this.client.vaultsList(params?.filterKey);
        logger.debug('VAULT-HANDLER', `vaultsList result: ${JSON.stringify(result)}`);
        // Also check encryption status
        const encStatus = await this.client.encryptionGetStatus();
        logger.debug('VAULT-HANDLER', `Encryption status: ${JSON.stringify(encStatus)}`);
        return result;
    }
    ...
    async loadVaultMetadata(vaultId) {
        ...
        if (vault) {
            ...
            logger.debug('VAULT-HANDLER', `Stored vault metadata after init: ${JSON.stringify(vaultData)}`);
        }
    }
    ...
    async activeVaultList(params) {
        ...
        logger.debug('VAULT-HANDLER', `Before activeVaultList: ${JSON.stringify({
            encStatus,
            vaultsStatus,
            activeVaultStatus,
    ...
    async closeAllInstances() {
        ...
        try {
            ...
            logger.error('VAULT-HANDLER', 'ERROR', `Error checking vault status before closing instances: ${e.message}`);
        ...
        if (wasAuthenticated) {
            logger.log('VAULT-HANDLER', 'INFO', 'Desktop was authenticated, navigating to master password screen after extension exit');
            if (global.window) {
...
//# sourceMappingURL=VaultHandlers.js.map