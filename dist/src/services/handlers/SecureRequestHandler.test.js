...
describe('SecureRequestHandler.handle', () => {
    ...
    it('should process a valid secure request and return encrypted response', async () => {
        ...
        sessionManager.decryptWithSession.mockReturnValue(Buffer.from(JSON.stringify(decryptedRequest), 'utf8'));
        ...
        expect(sessionManager.recordIncomingSeq).toHaveBeenCalledWith('session123', 42);
        expect(sessionManager.decryptWithSession).toHaveBeenCalled();
        expect(mockMethodRegistry.execute).toHaveBeenCalledWith('testMethod', { foo: 'bar' }, { client: mockClient });
        expect(sessionManager.encryptWithSession).toHaveBeenCalled();
        expect(result).toBe('encryptedResponse');
        expect(logger.debug).toHaveBeenCalledWith('SECURE-REQUEST', 'Received method: testMethod');
    });
...
//# sourceMappingURL=SecureRequestHandler.test.js.map