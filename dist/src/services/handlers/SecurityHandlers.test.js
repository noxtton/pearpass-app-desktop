...
describe('SecurityHandlers', () => {
    ...
    describe('nmGetAppIdentity', () => {
        it('throws if pairingToken is missing', async () => {
            await expect(handlers.nmGetAppIdentity({})).rejects.toThrow(/PairingTokenRequired/);
        });
        it('throws if verifyPairingToken returns false', async () => {
            ...
            await expect(handlers.nmGetAppIdentity({ pairingToken: 'token' })).rejects.toThrow(/InvalidPairingToken/);
        });
    ...
    describe('nmBeginHandshake', () => {
        ...
        it('throws if native messaging is disabled', async () => {
            ...
            await expect(handlers.nmBeginHandshake({ extEphemeralPubB64: 'abc' })).rejects.toThrow(/NativeMessagingDisabled/);
        });
        it('throws if extEphemeralPubB64 is missing', async () => {
            await expect(handlers.nmBeginHandshake({})).rejects.toThrow(/Missing extEphemeralPubB64/);
        });
    ...
    describe('nmFinishHandshake', () => {
        it('throws if sessionId is missing', async () => {
            await expect(handlers.nmFinishHandshake({})).rejects.toThrow(/Missing sessionId/);
        });
        it('throws if session not found', async () => {
            sessionStore.getSession.mockReturnValue(undefined);
            await expect(handlers.nmFinishHandshake({ sessionId: 'sid' })).rejects.toThrow(/SessionNotFound/);
        });
    ...
    describe('nmCloseSession', () => {
        it('throws if sessionId is missing', async () => {
            await expect(handlers.nmCloseSession({})).rejects.toThrow(/Missing sessionId/);
        });
...
//# sourceMappingURL=SecurityHandlers.test.js.map