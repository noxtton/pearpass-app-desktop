...
import { getOrCreateIdentity, getPairingCode, getFingerprint, __getMemIdentity } from './appIdentity';
...
describe('appIdentity', () => {
    ...
    describe('getOrCreateIdentity', () => {
        it('should initialize encryption if not initialized', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('APP-IDENTITY', 'Encryption not initialized, initializing...');
        });
        ...
        it('should handle encryption initialization failure gracefully', async () => {
            ...
            mockClient.encryptionAdd.mockResolvedValue();
            await getOrCreateIdentity(mockClient);
            expect(logger.info).toHaveBeenCalledWith('APP-IDENTITY', expect.stringContaining('Status check failed'));
            expect(mockClient.encryptionInit).toHaveBeenCalled();
        });
        it('should ignore "already initialized" errors', async () => {
            ...
            mockClient.encryptionAdd.mockResolvedValue();
            await getOrCreateIdentity(mockClient);
            expect(logger.error).not.toHaveBeenCalledWith('APP-IDENTITY', expect.stringContaining('already initialized'));
        });
    });
...
//# sourceMappingURL=appIdentity.test.js.map