...
export const getOrCreateIdentity = async (client) => {
    ...
    try {
        ...
        if (!initialized) {
            ...
            logger.info('APP-IDENTITY', `Encryption initialization result: ${JSON.stringify(initResult)}`);
        }
    }
    catch (err) {
        // If status check fails, try to initialize anyway
        logger.info('APP-IDENTITY', `Status check failed, attempting initialization: ${err.message}`);
        try {
            const initResult = await client.encryptionInit();
            logger.info('APP-IDENTITY', `Encryption initialization result: ${JSON.stringify(initResult)}`);
        }
            ...
            if (!initErr?.message?.includes('already initialized')) {
                logger.error('APP-IDENTITY', `Failed to initialize encryption: ${initErr.message}`);
            }
        }
    }
    // Try load encrypted blobs first (normalize to base64 string)
    const ed25519BlobB64 = normalizeEncryptionGet(await client.encryptionGet(ENC_KEY_ED25519).catch(() => null));
    const x25519BlobB64 = normalizeEncryptionGet(await client.encryptionGet(ENC_KEY_X25519).catch(() => null));
    ...
    if (!ed25519BlobB64 || !x25519BlobB64) {
        ...
        ed25519PublicKeyBytes = new Uint8Array(ed25519Buffer.slice(0, sodium.crypto_sign_PUBLICKEYBYTES));
        ed25519PrivateKeyBytes = new Uint8Array(ed25519Buffer.slice(sodium.crypto_sign_PUBLICKEYBYTES, sodium.crypto_sign_PUBLICKEYBYTES + sodium.crypto_sign_SECRETKEYBYTES));
        const x25519Buffer = Buffer.from(x25519BlobB64, 'base64');
        x25519PublicKeyBytes = new Uint8Array(x25519Buffer.slice(0, sodium.crypto_box_PUBLICKEYBYTES));
        x25519PrivateKeyBytes = new Uint8Array(x25519Buffer.slice(sodium.crypto_box_PUBLICKEYBYTES, sodium.crypto_box_PUBLICKEYBYTES + sodium.crypto_box_SECRETKEYBYTES));
    }
...
export const resetIdentity = async (client) => {
    ...
    try {
        ...
        logger.error('APP-IDENTITY', `Failed to clear existing keys: ${err.message}`);
    }
